...
class Main extends CI_Controller {
	...
	public function index(){
		...
		$section_list = $this->mainmodel->getSection("3");
		$i=0;
		$arr_page_section = "";
		if ( count($section_list) > 0 && $section_list !== ""){
			foreach ( $section_list as $data_section_list ){
				if ( $section_list[$i]['sec_type'] == "H" ){
					$arr_page_section[$i] = array( "sec_type" => $section_list[$i]['sec_type'], "html" => $section_list[$i] , "slider" => "", "slider_type" => $section_list[$i]['slider_type']);
				}else{
					$id_section = $section_list[$i]['id'];		
					$get_slider_list = $this->mainmodel->getSliderListById($id_section);
					$arr_page_section[$i] = array( "sec_type" => $section_list[$i]['sec_type'], "html" => "", "slider" => $get_slider_list, "slider_type" => $section_list[$i]['slider_type']);
				}
